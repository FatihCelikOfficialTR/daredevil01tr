...
exports.run = (client, message, args) => {
  var Rol =
    ...
    message.guild.roles.find(Rol => Rol.name === args.slice().join(" ")) ||
  ...
  if (!message.mentions.roles.first())
    if (!args[0]) message.channel.send(rolYok);
  if (message.mentions.roles.first()) {
    ...
    var renkliDynoLogo =
      `https://color.dyno.gg/color/${Rol.hexColor.replace(
        "#",
        ""
      )}/960x960.png` || "";
    if (Rol.hexColor == "#000000") renkliDynoLogo = "";
    if (Rol.hexColor !== "#000000")
      renkliDynoLogo = `https://color.dyno.gg/color/${Rol.hexColor.replace(
        "#",
        ""
      )}/960x960.png`;
    const Mesaj = new RichEmbed()
      ...
      .addField("**Rolün ismi ne?**", Rol.name, true)
      .addField("**Rolün ID'si ne?**", Rol.id, true)
      .addField(
        "**Rol kaçıncı sırada?**",
        `Yukarıdan ${Number(message.guild.roles.size) -
          Number(Rol.position)}; aşağıdan ${Rol.position}. sırada.`,
        true
      )
      .addField(
        "**Rol etiketlenebiliyor mu?**",
        "Sen bu rolün bilgisini etiketleyerek aldığına göre...",
        true
      )
      .addField("**Rolün rengi ne?**", `${Renk}`, true)
      .addField("**Rolde kaç kişi bulunuyor?**", Rol.members.size, true)
    ...
    if (args[0]) {
      ...
      if (!Rol)
        Rol = message.guild.roles.find(
          Rol => Rol.name === args.slice().join(" ")
        );
      ...
      const Mesaj = new RichEmbed()
        ...
        .addField("**Rol etiketlenebiliyor mu?**", rolEtiketlenmesi, true)
        .addField("**Rolün rengi ne?**", `${Renk}`, true)
        .addField("**Rolde kaç kişi bulunuyor?**", Rol.members.size, true)
